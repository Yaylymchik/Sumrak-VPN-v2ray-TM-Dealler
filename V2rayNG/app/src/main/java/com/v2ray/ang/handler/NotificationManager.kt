...
object NotificationManager {
    ...
    private var currentDaysLine: String? = null
    ...
    fun showNotification(currentConfig: ProfileItem?) {
        ...
        currentDaysLine = ProfileRemarkParser.formatDaysLine(
            service,
        ...
        mBuilder = NotificationCompat.Builder(service, channelId)
            .setSmallIcon(SumraXNotificationIcons.smallIcon(service))
            .setLargeIcon(SumraXNotificationIcons.largeIcon(service))
            .setContentTitle(currentRemarks)
            .setSubText(currentDaysLine)
            .setContentText(currentDaysLine)
    ...
    private fun updateNotification(contentText: String?, proxyTraffic: Long, directTraffic: Long) {
        if (mBuilder != null) {
            getService()?.let { service ->
                mBuilder?.setSmallIcon(SumraXNotificationIcons.smallIcon(service))
                mBuilder?.setLargeIcon(SumraXNotificationIcons.largeIcon(service))
            }
            val displayText = buildNotificationText(contentText)
            mBuilder?.setContentTitle(currentRemarks)
            mBuilder?.setSubText(currentDaysLine)
            mBuilder?.setStyle(NotificationCompat.BigTextStyle().bigText(displayText))
            mBuilder?.setContentText(displayText)
            getNotificationManager()?.notify(NOTIFICATION_ID, mBuilder?.build())
        }
    }

    private fun buildNotificationText(speedText: String?): String {
        val days = currentDaysLine
        return when {
            !speedText.isNullOrBlank() && !days.isNullOrBlank() -> "$days\n$speedText"
            !speedText.isNullOrBlank() -> speedText
            !days.isNullOrBlank() -> days
            else -> ""
        }
    }
    ...
    private fun updateSpeedNotificationOnce(lastZeroSpeed: Boolean): Boolean {
        ...
        val proxyUplink = traffic.proxyUplink
        val proxyDownlink = traffic.proxyDownlink
        val directUplink = traffic.directUplink
        val directDownlink = traffic.directDownlink
        ...
        if (!zeroSpeed || !lastZeroSpeed) {
            ...
            if (monthlyTotals.totalBytes > 0L && service != null) {
                text.append(MonthlyTrafficTracker.formatMonthlyLine(service, monthlyTotals))
            }
