...
class SubSettingActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        addCustomDividerToRecyclerView(binding.recyclerView, this, R.drawable.custom_divider_glass)
        binding.recyclerView.adapter = adapter

        mItemTouchHelper = ItemTouchHelper(object : SimpleItemTouchHelperCallback(adapter) {
            override fun getMovementFlags(
                recyclerView: RecyclerView,
                viewHolder: RecyclerView.ViewHolder
            ): Int {
                val subId = adapter.getSubId(viewHolder.bindingAdapterPosition) ?: return 0
                if (adapter.isLocked(subId)) {
                    return 0
                }
    ...
    private inner class ActivityAdapterListener : BaseAdapterListener {
        override fun onEdit(guid: String, position: Int) {
            if (SubscriptionLock.isLocked(MmkvManager.decodeSubscription(guid))) {
        ...
        override fun onLockedEditAttempt() {
            toast(R.string.sumrax_sub_locked)
        }
