...
object SubscriptionUpdater {
    ...
    fun sync(
        ...
        MmkvManager.decodeSubscriptions().forEach { sub ->
            // Happ: when global auto-update is on, all enabled subscriptions update;
            // when off, each subscription uses its own autoUpdate flag.
            val run = if (SettingsManager.isSubscriptionAutoUpdateEnabled()) {
                sub.subscription.enabled && sub.subscription.url.isNotBlank()
            } else {
                sub.subscription.autoUpdate && sub.subscription.enabled && sub.subscription.url.isNotBlank()
            }
            scheduleOne(
                context = context,
                subId = sub.guid,
    ...
    fun syncOne(context: Context = AngApplication.application, subId: String) {
        ...
        val run = if (SettingsManager.isSubscriptionAutoUpdateEnabled()) {
            subItem.enabled && subItem.url.isNotBlank()
        } else {
            subItem.autoUpdate && subItem.enabled && subItem.url.isNotBlank()
        }
        scheduleOne(
            context = context,
            subId = subId,
    ...
    /**
     * Cancel all scheduled subscription auto-update tasks.
     */
