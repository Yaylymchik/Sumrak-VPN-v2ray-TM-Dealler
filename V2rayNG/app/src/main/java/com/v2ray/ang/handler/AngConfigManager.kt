...
object AngConfigManager {
    ...
    private fun parseBatchConfig(servers: String?, subid: String, append: Boolean): Int {
        try {
            ...
            if (configs.isNotEmpty()) {
                ...
                resolveSelectedProfileKey(matchKey, subid)?.let { MmkvManager.setSelectServer(it) }
            }
    ...
    private fun resolveSelectedProfileKey(matchKey: String?, subid: String): String? {
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_AUTO_SELECT_PROFILE, true)) {
            return ProfileAutoSelector.findBestProfileGuid(subid) ?: matchKey
        }
        return matchKey
    }
     ...
     * 1. Exact match: remarks + server + port + password
    ...
    fun updateConfigViaSub(it: SubscriptionCache): SubscriptionUpdateResult {
        try {
            ...
            val userAgent = SettingsManager.resolveSubscriptionUserAgent(it.subscription.userAgent)
    ...
    private fun importUrlAsSubscription(url: String): Int {
        ...
        subItem.remarks = uri.fragment ?: AppConfig.SUBSCRIPTION_IMPORT_SUB_REMARKS
        subItem.url = url
