...
object SettingsManager {
    ...
    fun isRoutingEnabled(): Boolean {
        return MmkvManager.decodeSettingsBool(AppConfig.PREF_ROUTING_ENABLED, false)
    }

    fun getDefaultUserAgent(): String {
        return MmkvManager.decodeSettingsString(AppConfig.PREF_DEFAULT_USER_AGENT)
            ?: AppConfig.DEFAULT_USER_AGENT
    }

    /**
     * Resolved HTTP User-Agent for subscription fetches (Happ-style global UA).
     */
    fun resolveSubscriptionUserAgent(subscriptionUserAgent: String? = null): String {
        val forceGlobal = MmkvManager.decodeSettingsBool(AppConfig.PREF_FORCE_GLOBAL_USER_AGENT, true)
        val raw = if (forceGlobal) {
            getDefaultUserAgent()
        } else {
            subscriptionUserAgent?.trim()?.takeIf { it.isNotEmpty() } ?: getDefaultUserAgent()
        }
        return com.v2ray.ang.util.UserAgentPresets.resolve(raw)
    }

    fun isSubscriptionAutoUpdateEnabled(): Boolean {
        return MmkvManager.decodeSettingsBool(AppConfig.PREF_SUBSCRIPTION_AUTO_UPDATE_ENABLE, true)
    }

    fun isSubscriptionUpdateOnLaunchEnabled(): Boolean {
        return MmkvManager.decodeSettingsBool(AppConfig.PREF_SUBSCRIPTION_UPDATE_ON_LAUNCH, true)
    }

    fun isAutoCheckUpdateEnabled(): Boolean {
        return MmkvManager.decodeSettingsBool(AppConfig.PREF_AUTO_CHECK_UPDATE, true)
    }

    fun getAppUpdateApiUrl(): String {
        return MmkvManager.decodeSettingsString(AppConfig.PREF_APP_UPDATE_API_URL)
            ?.trim()
            .orEmpty()
            .ifEmpty { AppConfig.APP_API_URL }
    }
    ...
    fun setConnectionMode(smart: Boolean) {
        MmkvManager.encodeSettings(
            AppConfig.PREF_CONNECTION_MODE,
            if (smart) AppConfig.CONNECTION_MODE_SMART else AppConfig.CONNECTION_MODE_MANUAL
        )
    }
    ...
    private fun ensureDefaultSettings() {
        ...
        ensureDefaultValue(AppConfig.PREF_VPN_INTERFACE_ADDRESS_CONFIG_INDEX, "0")
        ...
        ensureDefaultValue(AppConfig.PREF_HEV_TUNNEL_LOGLEVEL, "warn")
        ensureDefaultValue(AppConfig.PREF_MUX_CONCURRENCY, "8")
        ensureDefaultValue(AppConfig.PREF_MUX_XUDP_CONCURRENCY, "8")
        ensureDefaultValue(AppConfig.PREF_LANGUAGE, Language.RUSSIAN.code)
        ensureDefaultValue(AppConfig.PREF_UI_MODE_NIGHT, "2")
        ensureDefaultValue(AppConfig.PREF_PING_TYPE, "tcp")
        ensureDefaultValue(AppConfig.PREF_FONT_SIZE, "2")
        ensureDefaultValue(AppConfig.PREF_FRAGMENT_LENGTH, "50-100")
        ensureDefaultValue(AppConfig.PREF_FRAGMENT_INTERVAL, "10-20")
        ensureDefaultValue(AppConfig.PREF_NOISE_RAND, "10-20")
        ensureDefaultValue(AppConfig.PREF_NOISE_DELAY, "10-16")
        ensureDefaultValue(AppConfig.PREF_DEFAULT_USER_AGENT, AppConfig.DEFAULT_USER_AGENT)
        ensureDefaultValue(AppConfig.PREF_APP_UPDATE_API_URL, AppConfig.APP_API_URL)

        ensureDefaultBoolValue(AppConfig.PREF_ROUTING_ENABLED, false)
        ensureDefaultBoolValue(AppConfig.PREF_FORCE_GLOBAL_USER_AGENT, true)
        ensureDefaultBoolValue(AppConfig.PREF_SUBSCRIPTION_AUTO_UPDATE_ENABLE, true)
        ensureDefaultBoolValue(AppConfig.PREF_SUBSCRIPTION_UPDATE_ON_LAUNCH, true)
        ensureDefaultBoolValue(AppConfig.PREF_AUTO_CHECK_UPDATE, true)
        ensureDefaultBoolValue(AppConfig.PREF_AUTO_SORT_AFTER_TEST, true)
        ensureDefaultBoolValue(AppConfig.PREF_AUTO_SELECT_PROFILE, true)
        ensureDefaultBoolValue(AppConfig.PREF_NOISE_ENABLED, false)
        ensureDefaultBoolValue(AppConfig.PREF_PING_ON_LAUNCH, false)
        ensureDefaultBoolValue(AppConfig.PREF_SPEED_ENABLED, true)
        ensureDefaultBoolValue(AppConfig.PREF_CONFIRM_REMOVE, false)
        ensureDefaultBoolValue(AppConfig.PREF_START_SCAN_IMMEDIATE, true)
        ensureDefaultBoolValue(AppConfig.PREF_DOUBLE_COLUMN_DISPLAY, false)
        ensureDefaultBoolValue(AppConfig.PREF_GROUP_ALL_DISPLAY, true)
        ensureDefaultBoolValue(AppConfig.PREF_PROXY_SHARING, false)
        ensureDefaultBoolValue(AppConfig.PREF_DYNAMIC_SOCKS_PORT, false)
        ensureDefaultBoolValue(AppConfig.PREF_SOCKS_ENABLE_UDP, true)
        ensureDefaultBoolValue(AppConfig.PREF_APPEND_HTTP_PROXY, false)
        ensureDefaultBoolValue(AppConfig.PREF_IPV6_ENABLED, false)
        ensureDefaultBoolValue(AppConfig.PREF_PREFER_IPV6, false)
        ensureDefaultBoolValue(AppConfig.PREF_LOCAL_DNS_ENABLED, false)
        ensureDefaultBoolValue(AppConfig.PREF_FAKE_DNS_ENABLED, false)
        ensureDefaultBoolValue(AppConfig.PREF_USE_HEV_TUNNEL, false)
    }
