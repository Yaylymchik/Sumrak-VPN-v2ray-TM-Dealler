...
//    }
    ...
    private fun applyExcludedRoutes(builder: Builder) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) return
        ExcludedRoutesManager.getRoutes().forEach { route ->
            ExcludedRoutesManager.parseRoute(route)?.let { (address, prefix) ->
                try {
                    val ipPrefix = IpPrefix(InetAddress.getByName(address), prefix)
                    builder.excludeRoute(ipPrefix)
                } catch (e: Exception) {
                    LogUtil.e(AppConfig.TAG, "Failed to exclude route: $route", e)
                }
            }
        }
    }
