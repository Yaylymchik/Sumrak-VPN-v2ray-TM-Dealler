...
class RoutingSettingActivity : HelperBaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.switchRoutingEnabled.isChecked = SettingsManager.isRoutingEnabled()
        binding.switchRoutingEnabled.setOnCheckedChangeListener { _, isChecked ->
            MmkvManager.encodeSettings(AppConfig.PREF_ROUTING_ENABLED, isChecked)
            binding.recyclerView.isVisible = isChecked
            if (isChecked && MmkvManager.decodeRoutingRulesets().isNullOrEmpty()) {
                lifecycleScope.launch(Dispatchers.IO) {
                    SettingsManager.resetRoutingRulesetsFromPresets(this@RoutingSettingActivity, 0)
                    withContext(Dispatchers.Main) { refreshData() }
                }
            }
        }
        binding.recyclerView.isVisible = SettingsManager.isRoutingEnabled()

        refreshUserAgentSummary()
        binding.layoutUserAgent.setOnClickListener { showUserAgentPicker() }
    ...
    private val userAgentOptions: Array<out String> by lazy {
        resources.getStringArray(R.array.subscription_user_agent_entries)
    }

    private fun refreshUserAgentSummary() {
        binding.tvUserAgentSummary.text = SettingsManager.getDefaultUserAgent()
    }

    private fun showUserAgentPicker() {
        AlertDialog.Builder(this).setItems(userAgentOptions) { _, index ->
            val value = userAgentOptions[index]
            MmkvManager.encodeSettings(AppConfig.PREF_DEFAULT_USER_AGENT, value)
            refreshUserAgentSummary()
        }.show()
    }
