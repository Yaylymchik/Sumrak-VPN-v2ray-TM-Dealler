...
import com.v2ray.ang.dto.UrlContentResult
...
object HttpUtil {
    ...
    fun getUrlContentWithUserAgent(request: UrlContentRequest): String {
        return getUrlContentResultWithUserAgent(request).body
    }

    @Throws(IOException::class)
    fun getUrlContentResultWithUserAgent(request: UrlContentRequest): UrlContentResult {
    ...
    fun headRequest(request: UrlContentRequest): Boolean {
        val url = request.url ?: return false
        val client = buildOkHttpClient(request.timeout, request.httpPort, request.proxyUsername, request.proxyPassword, followRedirects = true)
        val requestBuilder = Request.Builder()
            .url(url)
            .head()
            .header("Connection", "close")
        applyUserAgent(request, requestBuilder)
        applyProxyAuth(request, requestBuilder)
        return try {
            client.newCall(requestBuilder.build()).execute().use { it.isSuccessful }
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "HEAD request failed: $url", e)
            false
        }
    }

    fun measureDownloadBytes(request: UrlContentRequest, maxDurationMs: Long): Long {
        val url = request.url ?: return 0L
        val client = buildOkHttpClient(request.timeout, request.httpPort, request.proxyUsername, request.proxyPassword, followRedirects = true)
        val requestBuilder = Request.Builder()
            .url(url)
            .get()
        ...
        return try {
            ...
            LogUtil.e(AppConfig.TAG, "Failed to measure download: $url", e)
            0L
        }
    }

    fun measureUploadBytes(request: UrlContentRequest, payloadSize: Int): Pair<Long, Long> {
        val url = request.url ?: return 0L to 0L
        val client = buildOkHttpClient(request.timeout, request.httpPort, request.proxyUsername, request.proxyPassword, followRedirects = true)
        ...
        return try {
            ...
            client.newCall(requestBuilder.build()).execute().use { response ->
                ...
                if (!response.isSuccessful) return 0L to elapsed
                payloadSize.toLong() to elapsed
            }
        } catch (e: Exception) {
            LogUtil.e(AppConfig.TAG, "Failed to measure upload: $url", e)
            0L to 0L
        }
    }

    private fun applyUserAgent(request: UrlContentRequest, requestBuilder: Request.Builder) {
        if (!request.userAgent.isNullOrBlank()) {
            requestBuilder.header("User-Agent", request.userAgent)
        }
    }

    private fun applyProxyAuth(request: UrlContentRequest, requestBuilder: Request.Builder) {
        if (request.httpPort != 0 && !request.proxyUsername.isNullOrBlank() && !request.proxyPassword.isNullOrBlank()) {
            requestBuilder.header("Proxy-Authorization", Credentials.basic(request.proxyUsername, request.proxyPassword))
        }
    }
