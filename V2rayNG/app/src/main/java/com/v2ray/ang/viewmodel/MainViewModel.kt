...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var pendingPingOnComplete: ((String) -> Unit)? = null
    private var pingBatchJob: Job? = null

    companion object {
        const val PING_CANCELLED = "__ping_cancelled__"
    }
    ...
    fun reloadServerList() {
        ...
        updateCache()
        // Only fill selection when none is set. Smart Connect applies best after scan;
        // Manual mode must keep the user's pick across reloads.
        if (MmkvManager.getSelectServer().isNullOrEmpty()) {
            ProfileAutoSelector.applyBestSelection(subscriptionId)
        }
    ...
    /**
     * Tests TCP ping for all servers with completion callback and auto-sort.
     */
    fun testAllTcpingWithSort(onStatus: (String) -> Unit, onComplete: (String) -> Unit) {
        pingBatchJob?.cancel()
        tcpingTestScope.coroutineContext[Job]?.cancelChildren()
        SpeedtestManager.closeAllTcpSockets()
    ...
    /**
     * Cancels in-flight TCP / real-ping batch tests.
     */
    fun cancelAllPingTests() {
        pingBatchJob?.cancel()
        pingBatchJob = null
        tcpingTestScope.coroutineContext[Job]?.cancelChildren()
        SpeedtestManager.closeAllTcpSockets()
        ...
        val callback = pendingPingOnComplete
        pendingPingOnComplete = null
        callback?.invoke(PING_CANCELLED)
    }
    ...
    fun onTestsFinished() {
        viewModelScope.launch(Dispatchers.Default) {
            ...
            withContext(Dispatchers.Main) {
                ...
                pendingPingOnComplete = null
                callback?.invoke(
                    getApplication<AngApplication>().getString(R.string.result_ping_complete)
                )
            }
        }
    }
